...
func (ki *keyInfo) Update(bli *BlockListInterface, key string, data []byte) error {
	...
	if ki.Data == nil {
		save = true
		err, ki.Data = bli.GetFree(datasize)
		if err != nil { return err }
	} else if datasize != ki.Data.Entry.Size {
		err, ki.Data, _ = bli.Resize(ki.Data, datasize)
		if err != nil { return err }
		save = true
	}
...
type KeyHandler struct{
	//This class actually sets/gets/deletes a key from the database
...
func (kh *KeyHandler) makeNewList() error {
	...
	if !ok { return errors.New("Invalid type for headerinfo in makenewlist:")}
...
func (kh *KeyHandler) readFile(start int64) error {
	...
	for i := int64(0); i < header.Size; i++ {
		...
		if entry.Free > 0 {
			...
			if !ok { return errors.New("keyhandler: readFile: Location not found for key")}
			databli, ok := kh.bli.BlockListInfos[entry.Dataloc]
			if !ok { return errors.New("keyhandler: readFile: Location not found for data")}

			err, data := keybli.ReadData(kh.bli.file)
			if err != nil { return err }
			key := string(*data)
...
func (kh *KeyHandler) Set(key string, data []byte) error {
	...
	if el == nil {
		...
		if err != nil { return err }
		el = kh.freeKeyInfos.Front()
		if el == nil { return errors.New("Unable to get a free key info after creating new") }

	}

	info, ok := el.Value.(*keyInfo)
	if !ok { return errors.New("Invalid type in freeKeyInfos list")}
...
func (kh *KeyHandler) Get(key string) (error, *[]byte) {
	//Gets the data contained at string
	info, ok := kh.datalocs[key]
	if !ok { return errors.New("keyhandler: Get: Key does not exist"), nil }
